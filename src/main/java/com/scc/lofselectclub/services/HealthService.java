...
public class HealthService {
    ...
    public HealthResponseObject getStatistics(int idClub) throws EntityNotFoundException {
        ...
        try {
            ...
            for (Map.Entry<TupleBreed,List<HealthStatistics>> _currentBreed : _allBreeds.entrySet()) {
            	...
            	Map<Integer,List<HealthStatistics>> _breedGroupByYear= _currentBreed.getValue()
            			.stream()
            	...
            	HealthBreed _breed = new HealthBreed()
            		.withId(_id)
            		.withName(_name)
	...
	private List<HealthTest> extractHealthTest(List<HealthStatistics> _list) {
		...
		Map<TupleMaladie,List<HealthStatistics>> _breedGroupByHealthResult= _list
    			.stream()
    	...
    	for (Map.Entry<TupleMaladie,List<HealthStatistics>> _breedByHealthTest : _breedGroupByHealthResult.entrySet()) {
    		...
    		double _total = _breedByHealthTest.getValue().stream().map(e -> e.getNbResultat()).reduce(0, (x, y) -> x + y);
	...
	private List<HealthVariety> extractVariety(List<HealthStatistics> _list, double _total) {
		...
		Map<TupleVariety,List<HealthStatistics>> _allVariety = _list
				.stream()
		...
		for (Map.Entry<TupleVariety,List<HealthStatistics>> _currentVariety : _allVariety.entrySet()) {
			...
			HealthVariety _variety = new HealthVariety()
					.withId(_id)
					.withName(_name)
