...
public class BirthService {
	...
	@HystrixCommand(fallbackMethod = "buildFallbackBirthList",
        ...
        try {
            ...
            for (Map.Entry<TupleBreed,List<BreederStatistics>> _currentBreed : _allBreeds.entrySet()) {
            	...
            	for (Map.Entry<Integer,List<BreederStatistics>> _breedOverYear : _breedGroupByYear.entrySet()) {
                	...
                	BreederStatistics sumBirth = _breedOverYear.getValue()
                			.stream()
                		    .reduce(new BreederStatistics(0, 0), (x, y) -> {
                		         return new BreederStatistics(
                		        		 x.getNbMale() + y.getNbMale()
                		        		 , x.getNbFemelle() + y.getNbFemelle());
                		    });
                	
                	long _qtity = _breedOverYear.getValue()
        					.stream()
            				.collect(Collectors.counting()
            		);  
                	
                	double prolificity = _breedOverYear.getValue().stream().findFirst().map(BreederStatistics::getProlificiteRace).orElse(0.0);
                	...
                	BirthBreedStatistics _breed = new BirthBreedStatistics()
            			.withYear(_year)
            	...
            	for (int i = 0; i < _serieYear.length; i++) {
            		BirthBreedStatistics _breed = new BirthBreedStatistics()
            			...
            			.withTotalOfLitter(0)
            			.withProlificity(0)
            			.withVariety(new ArrayList<BirthVariety>())
            			.withCotations(new ArrayList<BirthCotation>())
            		;	
            		_breedStatistics.add(_breed);
            	}

            	// Création de l'objet Race
            	BirthBreed _breed = new BirthBreed()
            		.withId(_id)
            		.withName(_name)
            ...
            // Réponse
        	return new BirthResponseObject()
            		.withBreeds(_breeds)
		...
		Map<Integer, Long> _cotations = _list
				...
				.collect(
		                Collectors.groupingBy(
		                		BreederStatistics::getCotationPortee, Collectors.counting()
		                )
		        );
		;
    ...
    private List<BirthVariety> extractVariety(List<BreederStatistics> _list) {
		...
		for (Map.Entry<TupleVariety,List<BreederStatistics>> _currentVariety : _allVariety.entrySet()) {
        	...
        	// Somme des chiots males, femelles, portée
        	BreederStatistics sumBirth = _currentVariety.getValue()
        			.stream()
        		    .reduce(new BreederStatistics(0, 0), (x, y) -> {
        		         return new BreederStatistics(
        		        		 x.getNbMale() + y.getNbMale()
        	...
        	long _qtity = _currentVariety.getValue()
					.stream()
    				.collect(Collectors.counting()
    		);  

        	double prolificity = _currentVariety.getValue().stream().findFirst().map(BreederStatistics::getProlificiteVariete).orElse(0.0);
			...
			BirthVariety _variety = new BirthVariety()
					.withId(_id)
					.withName(_name)
        			.withNumberOfMale(sumBirth.getNbMale())
        			.withNumberOfFemale(sumBirth.getNbFemelle())
