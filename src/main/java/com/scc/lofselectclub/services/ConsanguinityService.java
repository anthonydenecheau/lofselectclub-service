...
public class ConsanguinityService {
	...
	@HystrixCommand(fallbackMethod = "buildFallbackConsanguinityList",
    ...
    public ConsanguinityResponseObject getStatistics(int idClub) throws EntityNotFoundException {
        ...
        logger.debug("In the ConsanguinityService.getStatistics() call, trace id: {}", tracer.getCurrentSpan().traceIdString());
        ...
        try {
            ...
            for (Map.Entry<TupleBreed,List<BreederStatistics>> _currentBreed : _allBreeds.entrySet()) {
            	...
            	for (Map.Entry<Integer,List<BreederStatistics>> _breedOverYear : _breedGroupByYear.entrySet()) {
                	...
                	double _cng = _breedOverYear
                		.getValue()
                		.stream().mapToDouble(BreederStatistics::getConsanguinite).average().orElse(0.0)
                	;
                	
                	// Moyenne du nb d'ancètres communs
                	List<ConsanguintyCommonAncestor> _commonAncestors = extractCommonAncestors(_breedOverYear.getValue());
                	...
                	ConsanguinityBreedStatistics _breed = new ConsanguinityBreedStatistics()
            			.withYear(_year)
            	...
            	ConsanguinityBreed _breed = new ConsanguinityBreed()
            		.withId(_id)
            		.withName(_name)
            ...
            // Réponse
        	return new ConsanguinityResponseObject()
            		.withBreeds(_breeds)
	...
	private ConsanguinityResponseObject buildFallbackConsanguinityList(int idClub){
    	...
    	list.add(new ConsanguinityBreed()
                .withId(0))
    	;
	...
	private List<ConsanguinityVariety> extractVariety(List<BreederStatistics> _list) {
		...
		for (Map.Entry<TupleVariety,List<BreederStatistics>> _currentVariety : _allVariety.entrySet()) {
        	...
        	ConsanguinityVariety _variety = new ConsanguinityVariety()
					.withId(_id)
					.withName(_name)
					.withCng(format.format(Precision.round(_cng,2)))
        			.withLitterByCommonAncestor(_commonAncestors)
	...
	private List<ConsanguintyCommonAncestor> extractCommonAncestors(List<BreederStatistics> _list) {
    	...
    	Map<Integer, Integer> _breedAndCommonAncestor = _list
    			.stream()
    			.collect(
    				Collectors.groupingBy(
    	        		BreederStatistics::getNbAncetreCommun,
    	...
    	// il faut maintenant compter de min à max (nb d'ancêtres commun), le nombre de dossier
		...
		if (_series.size()>0)
			for (int i = 1; i <= highestKey; i++) {
    			...
    			if (_series.containsKey(i))
    				c = new ConsanguintyCommonAncestor()
    	    			.withNumberOfCommonAncestor(i)
    	    			.withNumberOfLitter(_series.get(i))
    	    		;	
    			else
    				c = new ConsanguintyCommonAncestor()
	    				.withNumberOfCommonAncestor(i)
	    				.withNumberOfLitter(0)
	    			;	
